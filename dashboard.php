<?php
// index.php - NimCredit Dashboard with Real-time Database Connection and Device Tracking
session_start();
ob_start();
...
function detectDeviceInfo() {
    ...
    // Additional headers for IP detection
    if (!empty($_SERVER['HTTP_X_FORWARDED_FOR'])) {
        $deviceInfo['ip_address'] = $_SERVER['HTTP_X_FORWARDED_FOR'];
    } elseif (!empty($_SERVER['HTTP_CLIENT_IP'])) {
        $deviceInfo['ip_address'] = $_SERVER['HTTP_CLIENT_IP'];
    }
...
// Track login session
function trackLoginSession($pdo, $user_id) {
    $deviceInfo = detectDeviceInfo();
    $session_id = session_id();
    $device_hash = hash('sha256', 
        $deviceInfo['browser'] . 
        $deviceInfo['platform'] . 
    ...
    $check_sql = "SELECT device_id FROM device_logins 
                  WHERE agent_id = :agent_id AND device_hash = :device_hash AND status = 'active'";
    ...
    if ($check_stmt->rowCount() > 0) {
        ...
        $update_sql = "UPDATE device_logins 
                       SET is_current = TRUE, 
                           last_seen = NOW(),
                           session_id = :session_id
                       WHERE agent_id = :agent_id AND device_hash = :device_hash";
        ...
        // Insert new device login
        $insert_sql = "INSERT INTO device_logins 
                      (agent_id, device_hash, device_name, device_type, 
                       browser, platform, ip_address, session_id, is_current) 
                      VALUES (:agent_id, :device_hash, :device_name, :device_type, 
                              :browser, :platform, :ip_address, :session_id, TRUE)";
        $insert_stmt = $pdo->prepare($insert_sql);
        $insert_stmt->execute([
            'agent_id' => $user_id,
            'device_hash' => $device_hash,
            'device_name' => $deviceInfo['platform'] . ' - ' . $deviceInfo['browser'],
    ...
    $update_others_sql = "UPDATE device_logins 
                         SET is_current = FALSE 
                         WHERE agent_id = :agent_id 
                         AND device_hash != :device_hash";
    $update_others_stmt = $pdo->prepare($update_others_sql);
...
// Get device login count for a user
function getUserDeviceCount($pdo, $user_id) {
    $sql = "SELECT 
               COUNT(*) as total_devices,
               SUM(CASE WHEN status = 'active' THEN 1 ELSE 0 END) as active_devices,
               SUM(CASE WHEN is_current = TRUE THEN 1 ELSE 0 END) as current_device,
               device_type,
...
// Get all active super admin devices
function getSuperAdminDevices($pdo) {
    $sql = "SELECT 
               dl.*,
               a.username,
               a.full_name,
               a.role,
               TIMESTAMPDIFF(MINUTE, dl.last_seen, NOW()) as minutes_ago
            FROM device_logins dl
            JOIN agents a ON dl.agent_id = a.agent_id
...
// Logout device
function logoutDevice($pdo, $device_id, $agent_id) {
    $sql = "UPDATE device_logins 
...
function updateLastActivity($pdo, $user_id) {
    $deviceInfo = detectDeviceInfo();
    $device_hash = hash('sha256', 
        $deviceInfo['browser'] . 
        $deviceInfo['platform'] . 
...
function getDashboardStats($pdo) {
    ...
    // Active Devices (All Users)
    $stmt = $pdo->query("SELECT COUNT(DISTINCT device_hash) as total FROM device_logins WHERE status = 'active'");
    $stats['active_devices'] = $stmt->fetch()['total'] ?? 0;
    
    // Super Admin Active Devices
    $stmt = $pdo->query("SELECT COUNT(DISTINCT dl.device_hash) as total 
                         FROM device_logins dl
                         JOIN agents a ON dl.agent_id = a.agent_id
                         WHERE dl.status = 'active' AND a.role = 'superadmin'");
    $stats['superadmin_devices'] = $stmt->fetch()['total'] ?? 0;
...
// Track login session
trackLoginSession($db, $_SESSION['user_id']);
...
$deviceInfo = getUserDeviceCount($db, $_SESSION['user_id']);

// Get super admin devices if user is super admin
$superAdminDevices = [];
if ($user['role'] == 'superadmin') {
    $superAdminDevices = getSuperAdminDevices($db);
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        .device-card { border-left: 4px solid #3b82f6; }
        .device-card.current { border-left-color: #10b981; background-color: #f0f9ff; }
    </style>
</head>
<body class="bg-gray-50">
        ...
        <div class="p-6">
            ...
            <div class="mb-8 p-4 bg-blue-800 bg-opacity-30 rounded-xl">
                ...
                <div class="mt-4 grid grid-cols-2 gap-2 text-xs">
                    ...
                    <div class="text-center p-2 bg-blue-800 bg-opacity-40 rounded">
                        <div class="font-bold">Devices</div>
                        <div><?php echo $deviceInfo['active_devices']; ?> active</div>
                    </div>
                </div>
                <!-- Device info -->
                <div class="mt-3 text-xs text-blue-300">
                    <i class="fas fa-laptop mr-1"></i>
                    <?php echo $deviceInfo['platforms'] ?? 'Web'; ?>
                    <?php if ($deviceInfo['current_device']): ?>
                    <span class="ml-2 badge badge-success">Current</span>
                    <?php endif; ?>
                </div>
                <!-- Active Devices Quick View -->
                <?php if ($deviceInfo['active_devices'] > 1): ?>
                <div class="mt-2 p-2 bg-blue-900 bg-opacity-40 rounded text-xs">
                    <i class="fas fa-exclamation-triangle text-yellow-400 mr-1"></i>
                    Active on <?php echo $deviceInfo['active_devices']; ?> devices
                </div>
                <?php endif; ?>
            </div>
            ...
            <nav class="space-y-2">
                ...
                <a href="devices.php" class="flex items-center space-x-3 p-3 rounded-xl hover:bg-blue-800 hover:bg-opacity-30">
                    <i class="fas fa-desktop w-5"></i>
                    <span>Devices</span>
                    <span class="ml-auto badge badge-warning"><?php echo $stats['active_devices']; ?></span>
                </a>
            ...
            <div class="mt-8 pt-8 border-t border-blue-700">
                ...
                <div class="space-y-2">
                    ...
                    <a href="manage-devices.php" class="flex items-center space-x-2 text-sm p-2 rounded-lg bg-purple-600 hover:bg-purple-700">
                        <i class="fas fa-desktop w-4"></i>
                        <span>Manage Devices</span>
                    </a>
                    <a href="export-data.php" class="flex items-center space-x-2 text-sm p-2 rounded-lg bg-indigo-600 hover:bg-indigo-700">
    ...
    <div class="lg:ml-64 min-h-screen">
                ...
                <div class="flex justify-between items-center">
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <div class="text-sm text-gray-600 hidden md:block">
                            ...
                            Devices: <?php echo $stats['active_devices']; ?> active
                        </div>
                        ...
                        <div class="relative">
                            <button id="notificationsBtn" class="p-2 rounded-full bg-gray-100 text-gray-600 hover:bg-gray-200">
                                ...
                                <span class="absolute -top-1 -right-1 w-5 h-5 bg-orange-500 text-white text-xs rounded-full flex items-center justify-center">
                                    <?php echo $deviceInfo['active_devices'] - 1; ?>
                                </span>
                                <?php endif; ?>
                            </button>
                        </div>
        ...
        <main class="p-6">
                ...
                <div class="stat-card bg-gradient-to-r from-purple-500 to-pink-600 text-white rounded-2xl p-6">
                    ...
                    <div class="mt-4">
                        <div class="flex justify-between text-sm">
                            <span>Devices: <?php echo $stats['active_devices']; ?></span>
                            <span>Super Admin: <?php echo $stats['superadmin_devices']; ?></span>
                        </div>
                    </div>
                </div>
            ...
            <?php if ($user['role'] == 'superadmin' && !empty($superAdminDevices)): ?>
            <!-- Super Admin Devices Panel -->
            <div class="bg-white rounded-2xl shadow-sm p-6 mb-8">
                <div class="flex justify-between items-center mb-6">
                    <h3 class="text-lg font-semibold">Super Admin Active Devices</h3>
                    <span class="badge badge-info"><?php echo count($superAdminDevices); ?> active devices</span>
                </div>
                
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    <?php foreach ($superAdminDevices as $device): ?>
                    <div class="device-card <?php echo $device['is_current'] ? 'current' : ''; ?> border rounded-lg p-4 hover:bg-gray-50">
                        <div class="flex justify-between items-start mb-3">
                            <div class="flex items-center space-x-2">
                                <?php 
                                $deviceIcon = 'fa-desktop';
                                if ($device['device_type'] == 'mobile') $deviceIcon = 'fa-mobile-alt';
                                if ($device['device_type'] == 'tablet') $deviceIcon = 'fa-tablet-alt';
                                ?>
                                <div class="w-10 h-10 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center">
                                    <i class="fas <?php echo $deviceIcon; ?>"></i>
                                </div>
                                <div>
                                    <h4 class="font-medium"><?php echo htmlspecialchars($device['full_name']); ?></h4>
                                    <p class="text-xs text-gray-500">@<?php echo htmlspecialchars($device['username']); ?></p>
                                </div>
                            </div>
                        ...
                        <div class="space-y-2 text-sm">
                            <div class="flex justify-between">
                                ...
                                <span class="font-medium"><?php echo htmlspecialchars($device['platform']); ?></span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-600">Browser:</span>
                                <span><?php echo htmlspecialchars($device['browser']); ?></span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-600">IP Address:</span>
                                <span class="font-mono text-xs"><?php echo htmlspecialchars($device['ip_address']); ?></span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-600">Last Active:</span>
                                <span class="<?php echo $device['minutes_ago'] > 5 ? 'text-orange-600' : 'text-green-600'; ?>">
                                    <?php 
                                    if ($device['minutes_ago'] < 1) {
                                        echo 'Just now';
                                    } elseif ($device['minutes_ago'] < 60) {
                                        echo $device['minutes_ago'] . ' mins ago';
                                    } else {
                                        $hours = floor($device['minutes_ago'] / 60);
                        ...
                        <?php if ($user['agent_id'] == $device['agent_id'] && !$device['is_current']): ?>
                        <div class="mt-3 pt-3 border-t">
                            <button onclick="logoutDevice(<?php echo $device['device_id']; ?>)" 
                                    class="w-full py-1 text-xs text-red-600 hover:text-red-800 hover:bg-red-50 rounded">
        ...
        <footer class="bg-white border-t px-6 py-4">
            <div class="flex flex-col md:flex-row justify-between items-center">
                ...
                <div class="flex items-center space-x-4 mt-2 md:mt-0">
                    ...
                    <div class="text-sm text-gray-600">
                        <i class="fas fa-desktop mr-1"></i>
                        Active Devices: <?php echo $stats['active_devices']; ?>
                    </div>
                    <div class="text-sm text-gray-600">
            ...
            <div class="space-y-4">
                ...
                <div class="p-3 rounded-lg bg-orange-50 border border-orange-100">
                    <div class="flex items-start space-x-3">
                        ...
                        <div>
                            <p class="font-medium text-sm">Multiple Device Login</p>
                            <p class="text-xs text-gray-600 mt-1">Your account is active on <?php echo $deviceInfo['active_devices']; ?> devices</p>
                            <a href="manage-devices.php" class="text-xs text-orange-600 mt-1 inline-block hover:underline">
                                Manage Devices
                            </a>
                        </div>
                    </div>
